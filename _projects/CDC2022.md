...
a weighted least squares (WLS) method is used with a hyperbolic weighting matrix that leverages the commonly utilized log-normal shadowing path loss model 
\cite{goldsmith_2005}. Moreover, the RSSI-based position measurements have unknown covariance which differ from the nominal on-board position sensor. To 
...
unreliable state estimates (i.e., an improvement from \cite{AdaptiveR_EKF,AdaptiveR_UKF}) to optimize position estimation of any compromised agent.
